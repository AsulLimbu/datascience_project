\chapter{Actual Outcome}
        ...
        \begin{figure}[h]
                ...
                \caption{Energy Generation Mix and Interruption Analysis}
        \end{figure}

        \subsection{Energy Supply-Demand Balance}
        An important finding from the analysis is that the ``Energy not served/Generation Deficit'' column in the NEA reports contained all zeros throughout the study period (July 2022 -- March 2024). This indicates that Nepal successfully met its total daily energy requirements through a combination of domestic generation (NEA, NEA Subsidiaries, and IPPs) and imports from India.

        Instead of showing an uninformative zero-deficit chart, the analysis focuses on \textbf{Energy Interruption}, which represents temporary service disruptions due to grid issues, maintenance, or load shedding events. Key interruption statistics:
        \begin{itemize}
            \item \textbf{Total Interruption:} 372,269 MWh over the study period
            \item \textbf{Mean Daily:} 615 MWh
            \item \textbf{Maximum:} 4,285 MWh (April 20, 2023)
            \item \textbf{Days with Interruption:} 308 out of 606 days (51\%)
        \end{itemize}

        The interruption data reveals seasonal patterns, with higher values during winter months (December--February) when demand peaks and hydropower generation decreases, leading to more grid stress and maintenance-related outages.
        \pagebreak
